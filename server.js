...
io.on('connection', function (socket) {
    ...
    socket.on('SEND_MESSAGE', function (data) {
        ...
        if (
      (msgData == reponse && map.get(data.room).imageprogress < timeRound) ||
      (synonyms &&
        synonyms.some((e) => e.toLowerCase().latinize() == msgData) &&
      ...
      reponse.score(msgData) >= 0.8 ||
      synonyms &&
      synonyms.some((e) => e.toLowerCase().latinize().score(msgData) > 0.8)
    ) {
      message = data.user + " est proche !";
      io.sockets.in(data.room).emit("MESSAGE", { message: message });
    }
        else {
